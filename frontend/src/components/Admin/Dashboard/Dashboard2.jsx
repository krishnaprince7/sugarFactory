            ...
            <div className="bg-[#EAF1F8] rounded-xl p-6 relative flex-1 min-h-[435px]">
              <div className="flex flex-wrap gap-4">
                {[
                  ...
                  <div
                    key={factory.id}
                    className="bg-white rounded-xl shadow-sm w-[110px] h-[110px] flex flex-col items-center justify-center p-2 relative"
                  >
                    <div className="relative mb-2">
                      <MdLocationOn
                        className={`w-8 h-8 ${factory.status === "alert" ? "text-orange-500" : "text-green-500"}`}
                      />
                      <div
                        className={`absolute top-0 right-0 w-2.5 h-2.5 rounded-full ${factory.status === "alert" ? "bg-orange-500" : "bg-green-500"} border-2 border-white`}
                      />
                    </div>
                    ...
                    <span
                      className={`text-[10px] font-bold text-white px-2 py-0.5 rounded-full mt-1 ${factory.color} capitalize`}
                    >
                      {factory.status}
                    </span>
                  </div>
              ...
              {[
                ...
                >
                  ...
                  <div className="flex justify-between items-start pl-2">
                    ...
                    <span
                      className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${alert.statusBg}`}
                    >
                      {alert.status}
                    </span>
                  </div>
          ...
          <SectionCard title="Quick Analytics">
            <div className="flex flex-col gap-3 mt-2">
              {[
                ...
                <div
                  key={i}
                  className="flex justify-between items-center bg-[#F3F6F9] px-4 py-3.5 rounded-lg"
                >
                  <span className="text-sm font-medium text-gray-600">
                    {stat.label}
                  </span>
                  <span className={`text-base font-bold ${stat.valueColor}`}>
                    {stat.value}
                  </span>
                </div>
              ))}
