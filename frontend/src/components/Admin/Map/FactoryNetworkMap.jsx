                ...
                {factories.map((factory) => (
                  ...
                  >
                    <div className="relative inline-flex transform transition-transform group-hover:scale-110">
                      ...
                      />
                      <div
                        className={`absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${factory.status === "alert" ? "bg-amber-400" : "bg-emerald-500"}`}
                      />
                    </div>
                  </div>
                ))}
            ...
            <div className="lg:col-span-1 bg-white rounded-xl border border-slate-200 flex flex-col min-h-[300px] lg:min-h-[500px]">
              {selectedFactory ? (
                <div className="p-6 h-full flex flex-col">
                  ...
                  <div
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-medium w-fit ${selectedFactory.status === "alert" ? "bg-amber-50 text-amber-700 border border-amber-200" : "bg-emerald-50 text-emerald-700 border border-emerald-200"}`}
                  >
                    {selectedFactory.status === "alert"
                      ? "Alert Active"
                      : "Operational"}
                  </div>
